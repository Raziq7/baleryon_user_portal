...
export default function SignupModal({ onClose }: Props) {
  ...
  const validateSignUp = () => {
    ...
    if (!credentialsState.firstName.trim()) newErrors.firstName = "First name is required";
    if (!credentialsState.lastName.trim()) newErrors.lastName = "Last name is required";

    if (!/^\d{10}$/.test(credentialsState.mobileNo)) newErrors.mobileNo = "Enter a valid 10-digit mobile number";
    if (!credentialsState.gender) newErrors.gender = "Select a gender";
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(credentialsState.email)) newErrors.email = "Enter a valid email";

    const pw = credentialsState.password;
    const pwPattern = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@#$%^&*!?])[A-Za-z\d@#$%^&*!?]{8,}$/;
    if (!pw) newErrors.password = "Password is required";
    else if (!pwPattern.test(pw)) newErrors.password =
  ...
  const handleOtpSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    dispatch(verifyOtpThunk({ email: credentialsState.email, otp: credentialsState.otp }));
  };
            ...
            {otpRequired ? (
              <form onSubmit={handleOtpSubmit} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <InputOTP
                    ...
                    onChange={(value) => setCredentialsState((s) => ({ ...s, otp: value }))}
                    render={({ slots }) => (
                      <InputOTPGroup>
                ...
                <Button type="submit" className="w-full bg-black text-white hover:bg-gray-700" disabled={loading}>
                  Verify OTP
                </Button>
              </form>
            ) : (
              <form onSubmit={handleFormSubmit} className="space-y-4">
                <div>
                  ...
                  <Input value={credentialsState.firstName}
                    onChange={(e) => setCredentialsState((s) => ({ ...s, firstName: e.target.value }))}/>
                  {errors.firstName && <p className="text-sm text-red-500">{errors.firstName}</p>}
                </div>

                <div>
                  <Label>Last Name</Label>
                  <Input value={credentialsState.lastName}
                    onChange={(e) => setCredentialsState((s) => ({ ...s, lastName: e.target.value }))}/>
                  {errors.lastName && <p className="text-sm text-red-500">{errors.lastName}</p>}
                </div>

                <div>
                  <Label>Mobile Number</Label>
                  <Input type="tel" value={credentialsState.mobileNo}
                    onChange={(e) => setCredentialsState((s) => ({ ...s, mobileNo: e.target.value }))}/>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setCredentialsState((s) => ({ ...s, gender: value }))}
                  >
                ...
                <div>
                  <Label>Email</Label>
                  <Input type="email" value={credentialsState.email}
                    onChange={(e) => setCredentialsState((s) => ({ ...s, email: e.target.value }))}/>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Input
                      ...
                      onChange={(e) => setCredentialsState((s) => ({ ...s, password: e.target.value }))}
                      className="pr-10"
                    />
                ...
                <div>
                  ...
                  <Input type="password" value={credentialsState.confirmPassword}
                    onChange={(e) => setCredentialsState((s) => ({ ...s, confirmPassword: e.target.value }))}/>
